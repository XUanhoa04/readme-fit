...
export const structureRule: Rule = {
  ...
  evaluate: ({ readme, project }) => {
    ...
    const skipped = readme.headings.filter(
      (heading, index) =>
        index > 0 && heading.depth > (readme.headings[index - 1]?.depth ?? 0) + 1,
    );
    const normalized = readme.headings.map((heading) => heading.text.trim().toLowerCase());
    const duplicates = [
      ...new Set(normalized.filter((title, index) => normalized.indexOf(title) !== index)),
    ];
    const empty = readme.sections.filter((section) => section.wordCount === 0);
    const issues =
      (h1.length === 1 ? 0 : 1) + skipped.length + duplicates.length + empty.length;
    const evidence = [
      {
        type: 'h1-count',
        message: `${h1.length} H1 headings`,
        path: readme.path,
        value: h1.length,
      },
      {
        type: 'heading-skips',
        message: `${skipped.length} skipped levels`,
        path: readme.path,
        value: skipped,
      },
      {
        type: 'duplicate-headings',
        message: `${duplicates.length} duplicate titles`,
        path: readme.path,
        value: duplicates,
      },
      {
        type: 'empty-sections',
        message: `${empty.length} empty sections`,
        path: readme.path,
    ...
    return {
      ...
      findings: issues
        ? [
            finding({
              ...
              severity: 'low',
              priority: 'P3',
              title: 'README heading structure needs cleanup',
              ...
              impact:
                'Structural inconsistencies make scanning and generated navigation less predictable.',
              recommendation:
                'Use one H1, a sequential heading hierarchy, unique section titles, and remove empty sections.',
              evidence,
            }),
          ]
        : [],
      facts: {
        structure: {
          h1Count: h1.length,
          skippedHeadings: skipped.length,
