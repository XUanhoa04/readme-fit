...
export const runtimeRule: Rule = {
  ...
  evaluate: ({ repository, readme, project }) => {
    ...
    if (!readmeMatch?.[1] || !declared)
      return {
        score: naScore(
          'correctness.runtime.matches',
          'README or repository runtime constraint is absent.',
        ),
        findings: [],
      };
    ...
    return {
      ...
      findings: [
        finding({
          ...
          recommendation:
            'Align the README, package engines, and version file after confirming the supported runtime.',
          evidence: [
            {
              type: 'readme-runtime',
              message: `Node ${readmeMajor}`,
              path: readme.path,
              line,
            },
            {
