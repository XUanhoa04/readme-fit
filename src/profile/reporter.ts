...
const LABELS: Record<ProfileCategory, string> = {
  ...
  'technical-narrative': 'Technical Narrative',
  contact: 'Contact',
};

export function renderProfileTerminal(report: ProfileReport): string {
  const themes = Array.isArray(report.facts.visibleRepositoryThemes)
    ? (report.facts.visibleRepositoryThemes as Array<{
        theme: string;
        repositories: number;
      }>)
    : [];
  return (
    [
      ...
      ...Object.entries(report.scores).map(
        ([category, score]) =>
          `${LABELS[category as ProfileCategory].padEnd(22)}${`${score.score}/${score.maxScore}`.padStart(6)}`,
      ),
      '',
      ...
      '',
      ...(themes.length
        ? themes.map((theme) => `${theme.theme.padEnd(24)}${theme.repositories}`)
        : ['No recurring non-language themes detected.']),
      '',
      pc.bold('PRIORITIES'),
      '',
